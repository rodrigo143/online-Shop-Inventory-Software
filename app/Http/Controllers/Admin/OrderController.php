...
class OrderController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('admin.order.create',compact('unique'));
    }
    ...
    public function show($id)
    {
        ...
        if ($_REQUEST['status'] == 'All') {
            $orders  = $orders->latest('orders.id')->get();
             ...
             $orders  = $orders->where('orders.status', 'like', 'Completed')->orWhere('orders.status', 'like', 'Pending Invoiced')->latest('orders.id')->get();
         } else {
            $orders  = $orders->where('orders.status', 'like', $_REQUEST['status'])->latest('orders.id')->get();
        }
...
//                Notification::query()->truncate();
    ...
    public function status(Request $request)
    {
        ...
        if($order->courier_id || $status =='Canceled' || $status =='On Hold' ){
            $order->status = $status;
            $result = $order->save();
            if ($result) {
                $response['status'] = 'success';
